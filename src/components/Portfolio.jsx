...
const Portfolio = () => {
  ...
  const handleDialogue = () => {
    return (
      <Dialog
        ...
        modal={true}
        header="Project Demo:"
        headerStyle={{ fontWeight: 500, fontFamily: "Raleway" }}
        visible={visible}
        style={{
          fontFamily: "Raleway",
          width: "50vw",
        ...
        >
          {videoLink ? (
            <iframe
              className="demoVideo"
              style={{
                width: matchesSM ? "100%" : 330,
                height: matchesSM ? 315 : 250,
                borderRadius: 15,
              }}
              src={videoLink}
  ...
  return (
    <div
      name="portfolio"
      className="bg-gradient-to-b from-black to-gray-800 w-full text-white pt-20 flex items-center flex-col"
    >
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8 px-12 sm:px-0">
          {slice.map(({ id, src, demolink, videolink, codelink }) => (
            ...
            >
              <div
                key={id}
                className="shadow-md shadow-gray-600 rounded-lg overflow-hidden object-fit"
              >
                <img
                  src={src}
                  alt={`Project ${id}`}
                  className="rounded-md duration-200 ease-in hover:scale-110 w-full h-48 object-cover"
                />
                <div className="flex items-center justify-center p-4">
                  <button
                    onClick={() => handleDialogueState(demolink, videolink)}
                    className="w-1/2 px-6 py-3 m-4 bg-gradient-to-r
